...
for n in range(number_of_time_steps - 1):
	with tf.name_scope("Time_Step_{}".format(n)):
		...
		with tf.name_scope("Basket_Representation_{}".format(n)):
			...
			ordered_items 					= tf.squeeze(tf.slice(input_data,[0,n,0,0],[-1,1,-1,1]),[3])
			...
			user_factors 					= tf.slice(input_DSPO,[0,n+1],[-1,1])
...
def print_error():
	in_sample,DSPO_sample = data_loader.next_training_sample()
	if(in_sample.size > 0):
		in_sample = in_sample[:,data_time_step_start:,:,:]
		DSPO_sample = DSPO_sample[:,data_time_step_start:]
...
def val_error():
	...
	while(not val_data_loader.end_of_file):
		...
		in_sample,DSPO_sample = val_data_loader.next_training_sample()
		...
		if(in_sample.size > 0):
			...
			data_positives 	+= sess.run(tf.reduce_sum(ordered_items),feed_dict = {input_data:in_sample,input_DSPO:DSPO_sample})
			true_positives	+= sess.run(actual_positives,feed_dict = {input_data:in_sample,input_DSPO:DSPO_sample})
	print("Correct: {}, False Positives: {}, False Negatives: {}, Data Points: {}, True Positives: {}".format(correct,false_positives,false_negatives,data_positives,true_positives))
	# print(n)
